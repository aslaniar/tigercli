# BOOT BRIEF p2(124) - IDENTITY: DOES A CLIENT ASK FOR A PEER'S CHARACTER RECORD?

STATUS: live (2026-08-30). Reads with FINDINGS 20.196 (appearance does not cross the peer
channel), 20.195 (name content travels), 20.173 (the root->account gap).
...
BASELINE TO BEAT (p2(123), measured): 13 subscribe_in, 0 foreign roots.
...
SECOND CONTENT NEGATIVE: the identity bytes do NOT match the prediction. Then chunk 7's
encoding is wrong (it is `two read_wide(0x40)` per 20.179 R2, written here with the same
`write_raw_u64` the playerId field already uses successfully) and the subscription question
is unanswered this boot - the client was told nothing coherent.
...
off=192` line must BEGIN with account-then-character, little-endian:
    ...
    slot 1  000111000130AA9E 010111000130AA9E
...
CAVEAT: the character half assumes the account's SELECTED character is index 1
(primary+1, as the harvest shows). If a different character is selected the second qword
differs - the ACCOUNT half (the first 8 bytes) is the part with no such caveat, and the two
...
## DEPLOYED FOR THIS BOOT
  server exe     `222eb652a96471bb` - region A chunk 7 (account+character SOIDs) behind
                 server.profile_identity; all seven deploy gates passed, failures=0.
  ...
  settings       publish_player_profile TRUE, profile_name "SUNRISE",
                 ...
                 settings.json.bak_claude_p2d124.

## AFTER THE BOOT - REQUIRED
`bash RE_scripts/capture_boot.sh p2-124`
