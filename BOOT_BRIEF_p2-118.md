# BOOT BRIEF p2(118) - THE FIRST WRITE DETOUR: SUBSTITUTE THE NULL STAGING OBJECT

STATUS: live (2026-08-30). Reads with FINDINGS 20.189 (the p2(117) result) and 20.188.
...
0x141781800 invokes its profile helper, and `ev=ingress stage=apply` records a fire whose
`caller_rva` is NOT 0x17A6101.
...
- Zero `ev=ingress` lines at all: the profile_ingress observer is off; without it this boot
  cannot see its own answer. Confirm `client.profile_ingress = true` before booting.
...
## DEPLOYED FOR THIS BOOT
  ...
  MAC client DLL `0ac0d951d04db300` - staging detour ARMED-CAPABLE, null-only arming.
...
`bash RE_scripts/capture_boot.sh p2-118` BEFORE any other run starts.
