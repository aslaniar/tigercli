...
20.338 sharpens the latter: the construction chain is REGISTERED AND LIVE in the
p2-206 dump (heap descriptor carrying the exact registration arguments; the root
0x140B5ECD0's address in nine byte-identical records) - only the CONSTRUCTOR's
never-running is measured; whether the root executes-and-bails or is never
dispatched is the p2-211 call counter's question (staged, never ran).
