...
| 7 | **Ladder climbs to connected (4,5)** | 🔴 **THE FRONT - THE (3,4) STALL, RE-MEASURED p2-195.** The resv_rec probe, same boot, two records: rec=0 (the FORK's connection, ident ...0701A8C0) climbs to `s30e8=4 s1dc0=5` - ladder CONNECTED - with a real touch timestamp and mask flickering 0x0020. rec=1 (THE RIG's record, ident ...8801A8C0) goes `s1dc0=3` -> `4` and STOPS, mask=0x0000, touch=-1. Identical to the p2-180 measurement, so the stall survived W4, the duty-cycle fix and the whole relay arc. The 3->4 site 0x141803F2B requires the ladder at CONNECTED(5), so establishment cannot complete. The connected rung is MESSAGE-FED (session-plane chain 0x1417E5A10 -> pump -> 0x1416D56C0, event type 4 subtype != 8 -> 0x1416BCFC0): the fork's own connection receives it, the peer's record never does. NOTE: the membership dump's `s=_established/_connected` string is NOT this field - it is not even monotonic (`_connected` precedes `_joining`) - read s30e8/s1dc0 via resv_rec, never the dump string** | verified-by-execution (p2-180 and p2-195, resv_rec) + verified-by-reading (establishment-decode CLAIM 2) |
