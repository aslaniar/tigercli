...
| 8 | Guard + receiver object | 🔴 **THE BLOCKER — synthesis in (20.328); next = the type-51 echo boot** | The receive blocks were **never constructed — hard-confirmed** (0 ent_recv instances in all 4 dumps). **No wire message registers them** (lane C); no wire body can hit the type-9 step (lane A); the type-9 arm is structurally dead (20.327). **BUT type-51 is now implementable (lane B)**: the client's validator accepts a body whose decoded struct carries the client's OWN SteamNetworkingIdentity (`steamid:<id>#<token>` + 0x06) memcmp'd byte-exact — an ECHO of what the fork already captures (20.42), per-session (must read live), no secret. Its "apply half" (FUN_140B928D0) is the last candidate to drive the VMP gate byte or the setup-complete state. Next-boot contract: ONE passing type-51 behind a settings gate; readout = the ent_recv instance census (0 today) + the client census diff. Pre-boot work (no boot): femu the field→wire map, build the encoder, wire the live-token capture. | 20.328 + the three lane claims (session-identity-decode / type54-and-receiver-gate / type51-bubble-startup-spec) |
