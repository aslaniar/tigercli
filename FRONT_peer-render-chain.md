...
| 8 | Guard + receiver object | 🔴 **THE BLOCKER — p2-203 confounded; the re-test is next** | The receive blocks were **never constructed — hard-confirmed** (0 ent_recv instances in all four dumps). No wire message registers them directly (lane C); the type-9 step is structurally unreachable (lane A). **p2-203 sent the type-51 bubble-startup echo** (the 100%-femu-specified wire form, both identities captured live) — client silent, AND the pre-named account-mixup confounder fired: the rig's push echoed the WRONG identity (the capture keyed by account slot); only the mac's push was plausibly-correct — and still produced nothing visible. Two things decide the re-test: (a) the account-mapping fix (key the capture by the session's owner), (b) the boot-end dump check (the client's identity row vs the echo — "was the echo even right?" is today's unverified link). If a verified-correct echo STILL nulls, the apply half is inert and row 8 = the transport job-gate alone (ctor 0x1416BB1E0 ← job runner 0x1416CCDA0, the VMP +0x25 gate). | 20.328/20.329 + type51-bubble-startup-spec.md W1-W8 + p2-203 boot_outcome |
