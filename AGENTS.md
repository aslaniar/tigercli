...
| docs/archive/FRONT_public-host-chain.md | SUPERSEDED 2026-09-06 | road C's L1-L9 chain with its marks; the client-host premise was made moot by 20.311. Historical record only |
| docs/archive/FRONT_chain-to-a-moving-guardian.md | PREMISE RETRACTED (20.301) | the creation-loop/cond5/participant+0x38 front. Both halves of its premise are false - the loop never reads that byte. Read only for that history |
| FRONT_peer-render-chain.md | live front page (2026-09-06, PINNED CHART) | ALWAYS when tracking project status - the 10-row peer-render chain with statuses + evidence tokens; update after every boot/static verdict |
...
| docs/handoffs/HANDOFF_2026-09-03_CREATION.md | SUPERSEDED by 20.291 (front parked) | its PROVEN list (W1 closed, W3 down, cond5 a pure read, the walk skips self) stands and is still the right starting evidence; its QUESTION and its image/mask road are parked. Read FINDINGS 20.291 first |
| docs/handoffs/HANDOFF_2026-08-31_GATE-FEEDERS.md | SUPERSEDED by 2026-09-03_CREATION | ALWAYS when taking over. Its gate table + elimination list stand, but 20.246/20.250 spent its items 1-3 (static exhausted) and did item 4 (the p2-146 dump); STATE.md NEXT governs |
...
| docs/handoffs/HANDOFF_2026-08-31_POOL-PROTOCOL.md | SUPERSEDED by 20.219 | the pool-family STATIC MAP is still the reference; its hunt list and its "host mask empty" premise are dead |
| docs/handoffs/HANDOFF_2026-08-30_ENTITY-FRONT.md | superseded by 2026-08-31 | the entity-front hunt list (spent); its closed-fronts record stands |
| docs/handoffs/ (the 2026-08-27..08-30 series: LAYER-SHIFT, ROAD-C, L9-RENDER, ADMISSION, PROFILE-WRITER, STAGE-TRAY, CONSUMER-HUNT) | HISTORICAL, all superseded | only for the record of how each front was opened and closed; STAGE-TRAY and ADMISSION were both premise-false (retracted 20.191/20.170) |
| docs/ENFORCEMENT.md | live | the RULE->MECHANISM LEDGER: every hard rule -> gated/linted/template/manifest/rubric. READ before adding or converting any rule; incidents land as one row here, not prose |
| docs/handoffs/HANDOFF_2026-09-06_SESSION-BINDING.md | live | ALWAYS when taking over. Banks the 09-06 arc: the join delivery verified end-to-end, the identity fix hash-valid at +152, p2-187's live-blob instrument measured the lookup (the game's own refusal is greppable), and the open front = the session-to-connection binding. Artifacts index + the banked model review |
| HANDOFF_2026-09-08_EVENT-21.md | live - NEWEST HANDOFF, READ FIRST | ALWAYS when taking over. p2-212 measured: the executor evaluates OUR session (sid=2, stuck at 4 while the client's own reach 6), the C2 guard returns TRUE 9/9 on it, and a COMPLETE event enumeration shows EVENT 21 NEVER ARRIVES - the front is one missing message. recv_root ran 12,258x ("never dispatched" is dead). Client instruments COMMITTED (wt-p2211 @ 2f45faa = upstream-gameplay-scoped, pushed; phase02 merged to main 2026-09-08). Carries the WiFi settings re-point and the non-static next steps (diagnostic poke FIRST, then emit 21) |
| HANDOFF_2026-09-08_GATE-MACHINE.md | live (superseded as newest) | ALWAYS when taking over. Banks the gate-machine arc (20.339-20.348): the VMP wall retired, the manager found (state 4, armed, +0x8=2, empty pending queue), the activation job named and never dispatched, the attachment = world-change x LIVE-SESSION (6..9) - the session-state climb 4->6 reopened as the fork-side lever candidate. Corrections to 20.339/20.342 inline; the five lane reports banked; the six-value boot readout |
| HANDOFF_2026-09-07_PHASE01-P2211.md | live (superseded as newest by the 09-08 handoff) | ALWAYS when taking over. Banks the 09-07 phase-01 arc (20.336-20.338): the archive audit (3 of the last 4 boots logged startup-only; p2-207's record VOIDED), the vtable read (the construction chain is REGISTERED AND LIVE - only the constructor's never-running is measured), Stage B's inverted verdict + the churn finding (the 65:3 asymmetry = mac session lifetime), and the STAGED NEVER-RAN p2-211 boot (apply_p2211.py written, verified unapplied; C2 patch waiting) |
| HANDOFF_2026-09-06_NIGHT_INDEX-NAMESPACE.md | live (superseded as newest by the 09-07 handoff) | ALWAYS when taking over. Rows 1-7 fall (row 5 closed as host-only; row 7 passing; row 6 not a dependency) and the front moves to the ENTITY-INDEX NAMESPACE: idx_alloc 0x141711D10 returns -1 every boot. Carries the CLAIM O refutation (the svc21 body is a catalogue query, not an index request - do NOT build a grant onto it), two of my own claims disproved later the same session, the join-window instrument pattern, and the live-sampler rule |
| HANDOFF_2026-09-06_EVENING-CHANNEL.md | live - the evening arc (row 5) | ALWAYS when taking over. Banks the evening arc: the channel fix (the join gate walks [ctx+0x28] of the packet's OWN connection - relays now ride the ENGINE association), the gate's lookup MATCHING twice (p2-193a/b), and the SECOND check the decode had missed ([found_slot+0x1AEF8] in 6..9). Carries the four instrument defects and the exact next moves |
| docs/postmortems/POSTMORTEM_2026-09-06_THE-BLIND-GUARD.md | closed postmortem (+3 addenda) | THE INSTRUMENT ONE - READ BEFORE CHANGING ANY PROBE. Five boots measured nothing because the probe's own alignment guard refused the gate's (deliberately misaligned) blob pointers; then a novelty hash that erased every MATCH; then an enter-only hook on a LOOKUP; then a duplicate-RVA install that froze a client and a hot-path read count that stalled a landing. Rules R1-R8 |
| docs/handoffs/HANDOFF_2026-09-06_INSTRUMENT-WIDTH.md | live | the sess_cmp instrument-width gap for a workflow session: the shared helper measures the UNION of its callers, the novelty gate hides the refusal's own comparisons, and the spec for the caller-discriminated hook (per-triple gating + the replay acceptance) that answers which container the gate walks |
...
| docs/postmortems/POSTMORTEM_2026-08-31_THE-EMPTY-MASK-LOOP.md | closed postmortem | HOW ~8 BOOTS WENT TO A NON-BLOCKER: a return code read as a state, and 8 more mechanisms. Written for a workflow-fixing session; read before briefing any multi-boot front |
...
| docs/postmortems/POSTMORTEM_2026-09-05_THE-REBUILD.md | closed postmortem | THE SHIPPING ONE - READ BEFORE CHANGING A CLIENT HOOK OR SHIPPING A BUILD. ~9 launches, 3 measurements; 3 of 6 losses were defects I introduced: a BOM that broke the shim, an array whose hardcoded size I did not check (installed a hook at RVA 0 and crashed login), and a re-arm threshold unreachable by construction. All four share one shape: a change shipped without its own negative test. Also records what WORKED - fixture replay, input-gate-first reading, pre-named absence negatives |
