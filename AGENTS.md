...
| HANDOFF_2026-09-03_CREATION.md | live handoff | ALWAYS when taking over. The CREATION front: the peer's data is solved end to end and the blocker is how its record is CREATED - the guard's required bit is birth-set from the container. Carries what is proven, the one question, where to start statically, and the four things already paid for that must not be redone |
...
| FINDINGS 20.283-20.286 (in FINDINGS_2026-08-25.md) | newest verdicts | ALWAYS when taking over. W1 closed, W3 down, W2 dissolved (birth-set, not settable); three retractions on record |
