...
| RE_scripts/femu.py | FUNCTION EMULATION RIG: load the unpacked binary into a Unicorn x86-64 VM, call ONE function with crafted args, read the result. Import hooks = purity abort; instruction cap + crash hook fail loud. Interpreter: miniconda python3 (`unicorn` installed 08-29) | --selftest 12/12: image-map byte-exact, synthetic positives, timeout/crash/purity negatives, + REAL oracle: reason_name(0x1416E1620) returns file-table qwords (verified-by-execution) |
