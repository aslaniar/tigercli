...
| RE_scripts/gate_boot.py <brief> | mechanical pre-boot gate: the original 6 sections PLUS the v3 brief-tie fields (PRIOR ART with q.sh queries; STATE READERS rejecting 'inferred from'; EFFECT CLAIM; ABANDON OUTCOME; FIX SURFACE server/client with the U18 SERVER-SIDE GAP requirement) and, when INSTRUMENTS are declared, READOUT TRIGGER (prior-log cite or NEVER-OBSERVED), OBSERVER BUDGET, CALL FREQUENCY, HOOK COUNT (== verify_hook_rvas checked count), INSTRUMENT LIVENESS in brief-named SOURCE files | --literals fails loud on missing literal in deployed binary; third-branch streak >= 2 on the brief's FRONT refuses without a MODEL REVIEW section [caps: boot-gate, brief-tie, outcome-ledger] |
